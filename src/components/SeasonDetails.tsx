        ...
        <div className="p-4 max-w-6xl mx-auto">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm mb-6">
                ...
                {!myPicks?.picks || myPicks.picks.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {myPicks.picks.sort((a, b) => b.points - a.points).map((pick) => (
                            <div key={pick.id} className="bg-gray-50 p-4 rounded-lg">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className="font-medium text-gray-900">
                                            <span className="text-blue-600">{pick.selectedTeam}</span>
                                            {pick.spread && (
                                                <span className="ml-2">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm mb-6">
                ...
                {/* Check if current user has made all 3 picks */}
                {myPicks?.picks && myPicks.picks.length < 3 ? (
                    <div className="text-center py-8">
                        <p className="text-gray-500 mb-2">Other league members' selections will show up after you have selected all your games.</p>
                        <p className="text-sm text-gray-400">You have {myPicks?.picks?.length || 0}/3 picks selected.</p>
                    </div>
                ) : (
                    <>
                        {!leaguePicks?.userPicks || leaguePicks.userPicks.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {leaguePicks.userPicks.map((userPicks) => (
                                    <div key={userPicks.username} className="border border-gray-200 rounded-lg p-4">
                                        <h4 className="font-semibold text-gray-900 mb-3">{userPicks.username}</h4>
                                        {userPicks.picks.length === 0 ? (
                                            <p className="text-gray-500 text-sm italic">No selections yet</p>
                                        ) : (
                                            <div className="space-y-2">
                                                {userPicks.picks.sort((a, b) => b.points - a.points).map((pick) => (
